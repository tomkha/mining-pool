		...
		const routes = [
			{
				...
				runs : async (req, res, arr) => {
					...
					let totalEarned = (await poolServer.queryDB(QUERIES.minerTotalPayedOut, addr.toBase64())).map(it => it.amount);
					let totalOwed = (await poolServer.queryDB(QUERIES.minerTotalPayedOut, addr.toBase64())).map(it => it.amount);
